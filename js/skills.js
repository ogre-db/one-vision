        ...
        document.querySelectorAll('#weaponList tr:not(.spacer)').forEach( (element) => {
            ...
            element.addEventListener( 'click', function (event) {
                ...
                if ( weapon.dmgt || weapon.ele || weapon.rcbon ) {
                    ...
                    if ( weapon.dmgt > 0 ) {
                        infoDamageType.innerHTML = '<img data-src="' + damageTypes[weapon.dmgt]['icon'] + '"  width="40" height="40" uk-img>';
                        if (weapon.dmgtamt > 0) infoDamageType.innerHTML += '<br><b>' + weapon.dmgtamt + '%</b>';
                    } else infoDamageType.innerHTML = '';

                    if ( weapon.ele > 0 ) {
                        infoDamageElement.innerHTML = '<img data-src="' + elements[weapon.ele]['icon2'] + '"  width="40" height="40" uk-img>';
                        if (weapon.eleamt > 0) infoDamageElement.innerHTML += '<br><b>' + weapon.eleamt + '%</b>';
                    } else infoDamageElement.innerHTML = '';

                    if ( weapon.rcbon > 0 ) {
                        infoDamageRace.innerHTML = '<img data-src="' + races[isOdd(weapon.rcbon) ? (weapon.rcbon - 1) : weapon.rcbon]['icon'] + '"  width="40" height="40" uk-img>';
